...
public class ChapitreQuizzService implements IService<ChapitreQuizz> {
    ...
    public void createTableIfNotExist() {
        ...
        {
            String createTableQuery = "CREATE TABLE IF NOT EXISTS chapitreQuizz ( " +
    ...
    public void add(ChapitreQuizz instance) {
        Connection cnx = DatabaseConnection.getInstance().getCnx();
        String query = "INSERT INTO chapitreQuizz (questions, answers, correctAnswers, chapitreId) VALUES (?, ?, ?, ?)";
    ...
    public void update(ChapitreQuizz instance) {
        Connection cnx = DatabaseConnection.getInstance().getCnx();
        String query = "UPDATE chapitreQuizz SET " +
                "questions=?, " +
                "answers=?, " +
                "correctAnswers=?, " +
                "chapitreId=? " +
    ...
    public void delete(int id) {
        Connection cnx = DatabaseConnection.getInstance().getCnx();
        String query = "DELETE FROM chapitreQuizz WHERE id=?";
        try (PreparedStatement pst = cnx.prepareStatement(query)) {
            pst.setInt(1, id);
            pst.executeUpdate();
    ...
    public List<ChapitreQuizz> getAll() {
        ...
        String query = "SELECT * FROM chapitreQuizz";

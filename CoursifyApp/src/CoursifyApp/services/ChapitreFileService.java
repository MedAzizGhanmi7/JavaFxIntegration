...
public class ChapitreFileService implements IService<ChapitreFile> {
    ...
    public void createTableIfNotExist() {
        ...
        {
            String createTableQuery = "CREATE TABLE IF NOT EXISTS chapitreFile ( " +
    ...
    public void add(ChapitreFile instance) {
        Connection cnx = DatabaseConnection.getInstance().getCnx();
        String query = "INSERT INTO chapitreFile (id, name, content, chapitreId) VALUES (?, ?, ?, ?)";
    ...
    public void update(ChapitreFile instance) {
        ...
        Connection cnx = DatabaseConnection.getInstance().getCnx();
        String query = "UPDATE chapitreFile SET " +
                "name=?, " +
                "content=?, " +
                "chapitreId=? " +
                "WHERE id=?";
    ...
    public void delete(int id) {
        Connection cnx = DatabaseConnection.getInstance().getCnx();
        String query = "DELETE FROM chapitreFile WHERE id=?";
        try (PreparedStatement pst = cnx.prepareStatement(query)) {
            pst.setLong(1, id);
            pst.executeUpdate();
    ...
    public List<ChapitreFile> getAll() {
        ...
        String query = "SELECT * FROM chapitreFile";

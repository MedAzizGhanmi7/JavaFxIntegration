...
public class ChapitreService implements IService<Chapitre> {
    ...
    public void createTableIfNotExist() {
        ...
        {
            String createTableQuery = "CREATE TABLE IF NOT EXISTS chapitre (" +
    ...
    {
        Connection cnx = DatabaseConnection.getInstance().getCnx();
        String query = "INSERT INTO chapitre (name, description, creationDate, type, courseId) VALUES (?, ?, ?, ?, ?)";
    ...
    public void fullDelete(Chapitre c)
    {
        ...
        {
            ChapitreQuizz quizz = ChapitreQuizzService.getInstance().getOneByChapitreId(c.getId());
            if(quizz != null) ChapitreQuizzService.getInstance().delete(quizz.getId());
        }
        else
        {
            ChapitreFile file = ChapitreFileService.getInstance().getOneByChapitreId(c.getId());
    ...
    {
        Connection cnx = DatabaseConnection.getInstance().getCnx();
        String query = "UPDATE chapitre SET " +
                "name=?, " +
                "description=?, " +
                "creationDate=?, " +
                "type=?, " +
